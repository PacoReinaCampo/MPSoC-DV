...
4. Procedures should be established to ensure the integrity of the stored data for as long as required by the certification authorities by:
   ...
   1. Ensuring that no unauthorized changes are made.

   2. Selecting storage media.

   3. Maintaining availability of stored data. For example, by exercising or refreshing archived data at a frequency compatible with the storage life of the medium.

   4. Ensuring that a single event that can cause irretrievable loss of archived data is unlikely. For example, by storing duplicate copies in physically separate archives.
